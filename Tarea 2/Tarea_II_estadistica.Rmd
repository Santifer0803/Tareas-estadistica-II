...
Si cada $x_i, i \in \{ 1,..10 \}$ representa la cantidad de siniestros por periodo, se sabe que $x_i \sim Poisson(\lambda)$. Es decir, su verosimilitud viene dada por:

$$\prod_{i = 1}^{n} \mathbb{P}(x_i|\lambda) = \prod_{i=1}^{n}\frac{e^{-\lambda} \lambda^{x_i}}{(x_i)!} $$

Establecemos asi la verosimilitud:
...
La "propuesta" para $\lambda$ es que $\lambda \sim \gamma(3,2)$. Asi, procedemos con el algoritmo de Metropolis-Hastings:
...
Se calcula tambien la media de la muestra: 
...
## b)
Ahora se realiza la distribución de la muestra 
MCMC del algoritmo. 

```{r}
par(mfrow=c(2,2))
media=mean(mcmc)
curve(verosimilitud(x), from=1,to=3, type="l")
...
Ahora el grafico de autocorrelacion
...
Ahora el gráfico de la convergencia (promedios 
ergódicos) de la media de la muestra MCMC del algoritmo. 
...
Y finalmente, se calcula la tasa de aceptacion del algoritmo
```{r}
